...
const STEP_LABELS = {
  plan: 'Plan',
  profile: 'Profil (sidovy)',
} as const

const MODES: { id: AppMode; label: string }[] = [
  { id: 'grid', label: 'Rutnät' },
  { id: 'model3d', label: '3D-referens' },
]
...
export function AppHeader() {
  ...
  return (
    <header className="flex shrink-0 flex-wrap items-center gap-3 border-b border-border bg-surface-elevated px-4 py-2">
      <h1 className="text-lg font-semibold text-text">Skrovbyggaren</h1>
      ...
      {appMode === 'grid' ? (
        <>
          ...
          <p className="text-sm text-text-muted">Stormworks rutnät · 0,25 m/block</p>
        </>
      ) : (
        <p className="text-sm text-text-muted">Referensskrov från 3D-modell</p>
      )}
    </header>
  )
}
